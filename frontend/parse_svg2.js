...
function parseSVG(filename) {
    ...
    while ((match = rectRegex.exec(content)) !== null) {
        ...
        if (widthMatch && heightMatch) {
            ...
            // Look for TopBar in Desktop (width > 800)
            if (width >= 800 && height >= 50 && height <= 120) {
                console.log(`Found Desktop TopBar candidate: width=${width}, height=${height}, rx=${rxMatch?.[1]}, fill=${fillMatch?.[1]}`);
